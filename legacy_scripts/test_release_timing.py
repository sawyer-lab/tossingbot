...
class TrajectoryRecorder:
    def __init__(self, object_name, robot, gripper):
    ...
    def record_step(self, cmd_active=False):
        ...
        try:
            # Robot EE State
            ee_pose = self.robot.get_endpoint_pose()
            ee_vel = self.robot.get_endpoint_velocity()
            ...
            item = {
                ...
                "ee_pos": ee_pose['position'] if ee_pose else [0,0,0],
                "ee_vel": ee_vel['linear'] if ee_vel else [0,0,0]
            }
        ...
        except (rospy.ServiceException, KeyError, TypeError):
            pass
...
def execute_toss(robot, gripper, sol, release_idx, recorder, skip_gripper=False):
    ...
    for i in range(N):
        ...
        if i == release_idx and not skip_gripper:
            gripper.open()
            cmd_sent = True
...
def run_timing_test():
    ...
    recorder = TrajectoryRecorder("toss_cube", robot, gripper)
    ...
    TEST_SPEEDS = [2.0]
    TEST_OFFSETS = [4, 8, 12] 
    TEST_MODES = ["WITH_OBJECT", "WITH_OBJECT_NO_RELEASE", "NO_OBJECT_NO_RELEASE", "NO_OBJECT_WITH_RELEASE"]
    ...
    for mode in TEST_MODES:
        ...
        for speed in TEST_SPEEDS:
            for offset in TEST_OFFSETS:
                rospy.loginfo(f">>> SPEED: {speed:.1f} m/s | OFFSET: {offset}")
                
                for trial in range(TRIALS_PER_CONFIG):
                    # 1. Setup
                    if "WITH_OBJECT" in mode:
                        ...
                        hover_target = [pick_target[0], pick_target[1], pick_target[2] + 0.05]
                        execute_trajectory(robot, pick_planner.plan_cartesian(robot.get_joint_positions(), hover_target, [0,1,0,0], linear_speed=0.2))
                    ...
                    skip_grip = ("NO_RELEASE" in mode)
                    traj_data = execute_toss(robot, gripper, sol_7d, rel_idx_plan, recorder, skip_gripper=skip_grip)
                    
                    # 4. Analyze
                    land_pos, _ = sensor.get_landing_result(timeout=4.0 if mode == "WITH_OBJECT" else 0.1)
                    cmd_idx = next((i for i, x in enumerate(traj_data) if x['cmd_active']), None)
                    
                    delay_time, grip_v, flight_v, error_mm = 0.0, 0.0, 0.0, -1.0
                    
                    if cmd_idx is not None:
                        w_start = traj_data[cmd_idx]['grip_width']
                        motion_idx = next((i for i in range(cmd_idx, len(traj_data)) if abs(traj_data[i]['grip_width'] - w_start) > 0.001), None)
                        if motion_idx:
                            delay_time = traj_data[motion_idx]['t'] - traj_data[cmd_idx]['t']
                            # Note: we are recording both robot 'ee_vel' and cube 'vel'
                            # For loss, we use cube 'vel'
                            if 'vel' in traj_data[cmd_idx]:
                                grip_v = np.linalg.norm(traj_data[cmd_idx]['vel'])
                                flight_v = np.linalg.norm(traj_data[motion_idx]['vel'])
                                if land_pos:
                                    pred = get_ballistic_prediction(traj_data[motion_idx]['pos'], traj_data[motion_idx]['vel'])
                                    if pred is not None: error_mm = np.linalg.norm(np.array([land_pos.x, land_pos.y]) - pred) * 1000

                    full_log.append({
                        "mode": mode, "speed": speed, "offset": offset, 
                        "delay_ms": delay_time*1000, "loss": grip_v - flight_v,
                        "grip_v": grip_v, "flight_v": flight_v, "error_mm": error_mm,
                        "stop_idx": sol_3d["index"],
